...
class BlackJack:
    ...
    def draw_card(player:str, card_num:int, game:dict, amount: int = 1):
        ...
        # Calculate points
        points = card if isinstance(card, int) else (10 if card != "A" else (1 if game[player]['points_total'] + 11 > 21 else 11))
    ...
    def get_game_end_message_and_code(game_dict: dict, gettext: Callable):
        ...
        """
        _ = gettext
        player_points = game_dict['player']['points_total']
        dealer_points = game_dict['dealer']['points_total']
        player_busted = game_dict['player']['busted']
        dealer_busted = game_dict['dealer']['busted']
        ...
        if player_busted or dealer_busted:
            if player_busted and dealer_busted:
                ...
                message_end = _("Dealer busts. You received {amount} coins").format(amount=game_dict['player']['bet'] * 2)
                code = BlackJackEndCodes.PLAYER_WON
        else:
            if player_points > dealer_points:
                message_end = _("Dealer has less value than you. You received {amount} coins").format(amount=game_dict['player']['bet'] * 2)
                code = BlackJackEndCodes.PLAYER_WON
            elif player_points == dealer_points:
                message_end = _("It's a tie. You received your coins back")
                code = BlackJackEndCodes.TIE
            else:
                message_end = _("You have less value than dealer, and lost {amount} coins").format(amount=game_dict['player']['bet'])
                code = BlackJackEndCodes.DEALER_WON
    ...
    def create_blackjack_embed(game_dict: dict, outcome_message: str | None, gettext: Callable, status_code: BlackJackEndCodes) -> discord.Embed:
        ...
        embed.description = _("{nickname} bet **{bet}** coins to play blackjack").format(nickname=game_dict['player']['nickname'], bet=game_dict['player']['bet'])
        ...
        dealer_hand = ' '.join([BlackJack.format_card(card) for card in game_dict['dealer']['cards']]) if not dealer_first else BlackJack.format_card(game_dict['dealer']['cards'][0]) + bot_config.CustomEmojis.question_mark

        embed.add_field(name=f"{game_dict['player']['nickname']} [{game_dict['player']['points_total']}]", value=player_hand, inline=True)
        dealer_translation = _('Dealer')
        embed.add_field(name=f"{dealer_translation} [{game_dict['dealer']['points_total'] if not dealer_first else str(game_dict['dealer']['first_card_points']) + '+?'}]", value=dealer_hand, inline=True)
