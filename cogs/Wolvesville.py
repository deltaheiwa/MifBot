...
class WovClan(discord.ui.View):
    def __init__(self, clan_json_data, embed_func, api_caller, *, timeout=3600): # timeout = 1 hour
    ...
    async def repeat_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async with AsyncTranslator(JO.get_lang_code(interaction.user.id)) as at:
            ...
            if time_diff < 3600:
                ...
                await interaction.followup.send(content=_("Can't update information so frequently. Try again in **{timeleft}**").format(timeleft=pretty_time_delta(3600-time_diff)))
            else:
                self.json_data = await self.api_caller.add_to_queue(WovApiCall.get_clan_by_id, self.json_data['id'])
                self.json_data = await self.json_data
                member_list = await self.api_caller.add_to_queue(WovApiCall.get_clan_members, self.json_data['id'])
...
class WovPlayer(discord.ui.View):
    def __init__(self, embed_func, json_data, days, gettext, api_caller, *, timeout=3600): # timeout = 1 hour
        super().__init__(timeout=timeout)
        self.embed_func = embed_func
    ...
    async def sp_graph(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if self.json_data['id'] not in old_player_cache or len(old_player_cache[self.json_data['id']]) < 3:
            ...
            ax.set(title =self._("{user}' skill points over time").format(user=self.json_data['username']),
                xlabel = self._('Date'),
...
class ClanSelect(discord.ui.Select):
    def __init__(self, select_options, message_id) -> None:
        super().__init__(options=select_options, placeholder="Select a clan...", min_values=1, max_values=1)
...
class ClanSearchView(discord.ui.View):
    def __init__(self, select_options:list, ctx_message_id, user_called):
        ...
        self.add_item(ClanSelect(select_options=select_options, message_id=ctx_message_id))
    
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id == self.user.id:
            return True
        else:
            async with AsyncTranslator(JO.get_lang_code(interaction.user.id)) as translator:
                translator.install()
                _ = translator.gettext
                await interaction.response.send_message(_("You are not the one choosing! :pouting_cat:"), ephemeral=True)
            return False
    
    async def on_timeout(self) -> None:
        embed_err = discord.Embed(title="Timeout error", description="No clan number received", color = cfg.CustomColors.dark_red)
...
class Wolvesville(commands.Cog):
    ...
    async def wovclan(self, ctx, *, clan_name: str = None):
        async with AsyncTranslator(JO.get_lang_code(ctx.message.author.id)) as translator:
            translator.install()
            _ = translator.gettext
            ...
            if DF.cache_check("clan", "name", clan_name) is False:
                ...
                if len(clan_dict) > 1:
                    ...
                    for clan in clan_dict:
                        ...
                        select_options.append(discord.SelectOption(label=full_name, value=clan_dict.index(clan)+1))
                    view_chose = ClanSearchView(select_options=select_options,ctx_message_id=ctx.message.id,user_called=ctx.message.author)
            ...
            def embed_creation(dict_clan, description):
                ...
                embed_clan.add_field(name=_("Description"), value=clan_desc, inline=False)
                ...
                embed_clan.add_field(name=_("Minimum level to join"), value=dict_clan['minLevel'])
                embed_clan.add_field(name=_("Quests"), value=f"**{dict_clan['questHistoryCount']}**")
            ...
            if DF.cache_check(type_of_search="id", what_to_search="clan_members", name=dict_clan['id']) is True:
                ...
                member_list = await member_list
                DF.json_caching(cache_type="clan_members", json_data=member_list, extra_data=dict_clan['id'])
            ...
            for clan_member in member_list:
                ...
                if clan_member['isCoLeader'] is True:
                    # print(len(clan_leader_str + co_leader_str+f"\n{clan_member['level']} | **{clan_member['username']}** -*{clan_member['xp']:,}xp*"))
                    if len(clan_leader_str + co_leader_str+f"\n{clan_member['level'] if clan_member['level'] > 0 else '?'} | **{clan_member['username']}** - *{clan_member['xp']:,}xp*") > 1024:
    ...
    async def wovplayer(self, ctx:commands.Context, username: str = None, arg: str = None):
            ...
            if arg in ["avatars", "avatar"]:
                ...
                for avatar in attachments:
                    if attachments.index(avatar) == 0:
                        select_options.append(discord.SelectOption(label=_("All avatars")))
                        embed_names.append("All_avatars")
                    else:
                        select_options.append(discord.SelectOption(label=_("Avatar {num}").format(num=attachments.index(avatar))))
                ...
                view.message = await main_message.edit(embed=avatar_embed, attachments=[attachment], view=view)
            else:
                async def embed_creation(player_data):
                    ...
                    for item in ranked_keys:
                        match item:
                            case "rankedSeasonSkill":
                                ...
                                else: current_sp = _("*Didn't participate in current season or data is private*")
                            case "rankedSeasonMaxSkill":
                                if private_ranked:
                                    max_sp = _("**?**")
                                elif item in player_data and player_data[item] > 0:
                                    max_sp = f"**{player_data[item]}**"
                                else: max_sp = _("*Didn't participate in any season yet or data is private*")
                            case "rankedSeasonBestRank":
                                if private_ranked:
                                    best_rank = _("**?**")
                                elif item in player_data and player_data[item] > 0:
                                    best_rank = f"**{player_data[item]}**"
                                else: best_rank = _("*Didn't participate in any season yet or data is private*")
                            case "rankedSeasonPlayedCount":
                                if item in player_data and player_data[item] > 0:
                                    ...
                                    private_ranked = True
                    embedPlayer.add_field(name=_("Ranked stats"), value=_("Current sp: {}\nOverall best sp: {}\nBest season final rank: {}\nSeasons participated in: **{}**").format(
                        current_sp,
                        max_sp,
                        best_rank,
                        seasons_played
                    ))
                    if 'gameStats' in player_data and player_data['gameStats']['totalWinCount'] > 0:
                    ...
                    if player_data['gameStats']['villageWinCount'] >= 0:
                        ...
                        role_stats = _("Village: **{}** Wins/**{}** Defeats  **({}% wr)**\nWerewolves: **{}** Wins/**{}** Defeats  **({}% wr)**\nSolo voting: **{}** Wins/**{}** Defeats  **({}% wr)**\nSolo killer: **{}** Wins/**{}** Defeats  **({}% wr)**").format(
                    ...
                    embedPlayer.add_field(name=_("Team stats"), value=role_stats, inline=False)
                    try:
                    ...
                    if clan_id is not None:
                        ...
                        if clan_check is False:
                            ...
                            clan_dict_future = await self.api_caller.add_to_queue(WovApiCall.get_clan_by_id, player_data['clanId'])
                        ...
                        clan_name = surrogates.decode(clan_name)
                        clan_info = _("`{}` | **{}** :flag_{}: \n{}***...***\n*use `.wov-clan {}` for more information*").format(
                            c_tag,
                            clan_name,
                    ...
                    embedPlayer.add_field(name=_("Avatars"), value=_("*Use command `.wov-player {username} avatars` for avatars*").format(username=username))
                    if previous_username:
                        embedPlayer.add_field(name=_("Previous username"), value=previous_username)
                ...
                try:
                    ...
                    days = 30
                view = WovPlayer(embed_func=embed_creation, json_data=player_dict, days=days, gettext=_, api_caller=self.api_caller)
