...
from bot_util.bot_functions import CustomFormatter, AsyncTranslator, pretty_time_delta, coins_formula
...
class RegAccountExists(discord.ui.View):
    ...
    async def decline(self, interaction: discord.Interaction, button: discord.ui.Button):
        embed = discord.Embed(
            title=self._("You declined"), description=self._("If you want to login into your account, simply use the `login` command"),
...
class RegistrationModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if self.children[1].value != self.children[2].value:
            embedErr=discord.Embed(title=self._("Error"), description=self._("Passwords {password} and {confirm_password} don't match!").format(password=self.children[1].value, confirm_password=self.children[2].value), color=bot_config.CustomColors.dark_red)
            await interaction.response.send_message(embed=embedErr, ephemeral=True)
            return
    ...
    async def wallet(self, ctx: commands.Context):
        try:
            ...
            async with AsyncTranslator(JO.get_lang_code(user.id)) as lang:
                lang.install()
                _ = lang.gettext
                if DF.local_checks(user.id) is True:
        ...
        try:
            ...
            async with AsyncTranslator(JO.get_lang_code(user.id)) as lang:
                ...
                if info_about_daily['lastDailyTime'] is not None:
                    time_diff = dt.utcnow() - dt.strptime(info_about_daily['lastDailyTime'], "%Y-%m-%dT%H:%M:%S.%fZ") # type: ignore
                ...
                if time_diff > 75600*2:
                    ...
                    info_about_daily['lastDailyTime'] = dt.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%fZ")
