...
class Special(commands.Cog):
    ...
    async def launchBot(self, ctx):
        ...
        try:
            self.bot.telegram_bot = MifTelegramReporter()
    ...
    async def updateFile(self, ctx: commands.Context, path: str = './', *, args=None):
        ...
        if os.path.exists(f'{BACKUP_PATH}{attachment.filename}'):
            shutil.copyfile(f'{path}{attachment.filename}', f'{BACKUP_PATH}{backup_filename}')
    ...
    async def revert_file(self, ctx, filename: str, filepath: str = './'):
        ...
        try:
            if os.path.exists(f'{BACKUP_PATH}{backup_filename}'):
                shutil.copyfile(f'{BACKUP_PATH}{backup_filename}', f'{filepath}{filename}') # Find a path to the file in the directory
    ...
    async def database(self, ctx, arg, arg2=None, arg3=None):
        match arg:
            ...
            case "help":
                await ctx.reply("```database <mysql/local> <mif/mif_vg|game> <create|drop/init>```")
    ...
    @commands.hybrid_command(aliases=[], with_app_command=True, name="information", description="Get information about the bot", guilds=[b_cfg.main_guild_id])
    @dev_command()
    ...
    @commands.hybrid_command(aliases=["dir"], with_app_command=True, name="directory", description="Get the directory structure of the bot", guilds=[b_cfg.main_guild_id])
    @dev_command()
    ...
    @commands.hybrid_command(name="help-dev", description="Provides help information for developer commands", with_app_command=True, guilds=[b_cfg.main_guild_id], aliases=["helpDev", "helpD"])
    @dev_command()
    async def helpDev(self, ctx):
        ...
        embed.add_field(name="resourceUsage", 
        ...
        embed.add_field(name="updateFile <path> <*args>", 
                        ...
                        inline=False)
        embed.add_field(name="revert_file <filename> [filepath]", 
                        ...
                        inline=False)
        embed.add_field(name="database <type> <subcommand> [action]", 
                        ...
                        inline=False)
        embed.add_field(name="update [tabledb] [obj_id]", 
